...
public class AgendaFestivales {
    ...
    public LinkedHashMap<Estilo,TreeSet<String>> festivalesPorEstilo() {
        ...
        Iterator<Mes> it = claves.iterator();
        while (it.hasNext()) {
            Mes mes = it.next();
            ArrayList<Festival> festival = agenda.get(mes);
                for (Festival f : festival) {
                    HashSet<Estilo> estilos = f.getEstilos();
                    for (Estilo estilo : estilos) {
                        if (grupos.containsKey(estilo)) {
                            TreeSet<String> nombres = grupos.get(estilo);
                            nombres.add(f.getNombre());
                            grupos.put(estilo,nombres);
                        }
                        else {
